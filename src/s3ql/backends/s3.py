...
class Bucket(AbstractBucket):
    ...
    def __init__(self, bucket_name, aws_key_id, aws_key):
        ...
        self.namespace = 'http://oss.aliyun.com/doc/2011-03-01'
    ...
    def _get_conn(self):
       ...
       #return httplib.HTTPConnection('%s.s3.amazonaws.com' % self.bucket_name)          
        return httplib.HTTPConnection('storage.aliyun.com:8080')
    ...
    def _get_region(self):
       ...
       #resp = self._do_request('GET', '/', subres='location')
        resp = self._do_request('GET', '/', subres='')
        
        region = ElementTree.parse(resp).getroot().text.rstrip()
    ...
    def _add_auth(self, method, url, headers, subres=None, query_string=None):
       ...
       #for hdr in ('content-md5', 'content-type', 'date'):
        ...
        headers['authorization'] = 'OSS %s:%s' % (self.aws_key_id, signature)
       #headers['authorization'] = 'AWS %s:%s' % (self.aws_key_id, signature)
       #print("".join(auth_strs) + "\n" + headers['authorization'])
...
class ObjectR(object):
    ...
    def close(self):
        ...
        if etag.lower() != self.md5.hexdigest().lower():
